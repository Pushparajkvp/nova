            ...
            if ins:
                for i in ins:
                    if i is tsock:
                        try:
                            data = i.recv(8192)
                            LOG.info("GOT FROM TSOCK: " + len(data))
                            if data:
                        ...
                        try:
                            data = i.recv(8192)
                            LOG.info("GOT FROM HTTP: " + len(data))
                            if data:
                ...
                for i in ows:
                    if i is tsock and tsock_out:
                        try:
                            while tsock_out:
                                i.send(tsock_out[0])
                                LOG.info("Sent tsock : " + len(tsock_out[0]))
                                tsock_out.pop(0)
                        except BlockingIOError:
                        ...
                        try:
                            while http_out:
                                i.send(http_out[0])
                                LOG.info("Sent http : " + len(http_out[0]))
                                http_out.pop(0)
                        except BlockingIOError:
